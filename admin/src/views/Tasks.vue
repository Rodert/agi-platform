<template>
  <div class="tasks-page" v-loading="loading">
    ...
    <el-table :data="tasks" class="task-table">
      <el-table-column label="预览" width="96"><template #default="{ row }"><el-image v-if="row.type === 'image' && (row.thumbnail_url || row.result_url)" :src="row.thumbnail_url || row.result_url" fit="cover" class="preview" :preview-src-list="[row.result_url || row.thumbnail_url]" preview-teleported/><video v-else-if="row.type === 'video' && row.result_url" :src="row.result_url" class="preview" muted/><span v-else class="empty-preview">-</span></template></el-table-column>
    ...
    <el-dialog v-model="detailVisible" title="生成任务详情" width="720px"><template v-if="selected"><el-descriptions :column="2" border><el-descriptions-item label="任务 ID">{{ selected.id }}</el-descriptions-item><el-descriptions-item label="状态">{{ statusText(selected.status) }}</el-descriptions-item><el-descriptions-item label="用户">{{ selected.user_name }} {{ selected.user_email }}</el-descriptions-item><el-descriptions-item label="渠道">{{ selected.channel_name || '-' }}</el-descriptions-item><el-descriptions-item label="模型">{{ selected.model_name }}</el-descriptions-item><el-descriptions-item label="消耗">{{ selected.cost }} 灵感值</el-descriptions-item><el-descriptions-item label="执行次数">{{ selected.attempt_count }} 次</el-descriptions-item><el-descriptions-item label="重试次数">{{ retryCount(selected) }} / {{ selected.max_retry_attempts }}</el-descriptions-item><el-descriptions-item label="最近重试">{{ selected.last_retry_at || '-' }}</el-descriptions-item><el-descriptions-item label="完成时间">{{ selected.completed_at || '-' }}</el-descriptions-item><el-descriptions-item label="提示词" :span="2">{{ selected.prompt }}</el-descriptions-item><el-descriptions-item label="生成参数" :span="2"><pre>{{ JSON.stringify(selected.params || {}, null, 2) }}</pre></el-descriptions-item><el-descriptions-item v-if="selected.error_msg" label="最终失败原因" :span="2">{{ selected.error_msg }}</el-descriptions-item></el-descriptions><section v-if="selected.assets && selected.assets.length" class="attempts"><h3>对象存储资源</h3><el-table :data="selected.assets" size="small"><el-table-column label="类型" width="100"><template #default="{ row }">{{ resourceTypeText(row.resource_type) }}</template></el-table-column><el-table-column prop="object_key" label="对象 Key" min-width="240" show-overflow-tooltip/><el-table-column prop="size_bytes" label="大小" width="110"><template #default="{ row }">{{ fileSize(row.size_bytes) }}</template></el-table-column><el-table-column prop="expires_at" label="到期时间" width="170"/></el-table></section><section v-if="selected.attempts && selected.attempts.length" class="attempts"><h3>执行记录</h3><el-timeline><el-timeline-item v-for="attempt in selected.attempts" :key="attempt.attempt" :timestamp="attempt.started_at" :type="attemptType(attempt.status)">{{ attempt.attempt }} 次执行：{{ attemptText(attempt.status) }}<small v-if="attempt.error_msg">{{ attempt.error_msg }}</small></el-timeline-item></el-timeline></section></template></el-dialog>
  </div>
</template>
<script setup>
