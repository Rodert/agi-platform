...
func (h *AdminConfigHandler) ListChannels(c *gin.Context) {
	...
	for _, row := range rows { row.APIKey = "******" }
	response.Success(c, rows)
}

func (h *AdminConfigHandler) CreateChannel(c *gin.Context) {
	var req channelRequest
	if err := c.ShouldBindJSON(&req); err != nil || req.APIKey == "" { response.Error(c, errors.ErrBadRequest); return }
	...
	if err := h.providerRepo.Create(row); err != nil { response.Error(c, err); return }
	row.APIKey = "******"
...
func (h *AdminConfigHandler) UpdateChannel(c *gin.Context) {
	...
	if row.Priority <= 0 { row.Priority = 100 }
	if req.APIKey != "" && req.APIKey != "******" { row.APIKey = req.APIKey }
	if err := h.providerRepo.Update(row); err != nil { response.Error(c, err); return }
	row.APIKey = "******"
	response.Success(c, row)
}
...
func (h *AdminConfigHandler) DeleteChannel(c *gin.Context) {
	id, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err == nil { err = h.providerRepo.Delete(id) }
	if err != nil { response.Error(c, err); return }
...
func (h *AdminConfigHandler) UpdateChannelModelStatus(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil { response.Error(c, errors.ErrBadRequest); return }
	if err := h.channelModelRepo.UpdateStatus(channelID, modelID, req.IsActive); err != nil { response.Error(c, err); return }
	response.Success(c, nil)
}
