...
func (r *AIProviderAccountRepository) List() ([]*model.AIProviderAccount,error) { var rows []*model.AIProviderAccount; return rows,r.db.Preload("ChannelModels", "is_active = ?", true).Preload("ChannelModels.Model").Order("priority ASC, provider ASC, id ASC").Find(&rows).Error }
