...
func AuthMiddleware(cfg *config.JWTConfig, sessionRepo *repository.UserSessionRepository) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		session, err := sessionRepo.FindActive(claims.SessionID)
		if err != nil || session.UserID != claims.UserID {
			response.Error(c, errors.New(errors.ErrCodeUnauthorized, "登录已失效"))
			c.Abort()
			return
		}
...
func OptionalAuthMiddleware(cfg *config.JWTConfig, sessionRepo *repository.UserSessionRepository) gin.HandlerFunc {
	return func(c *gin.Context) {
		authHeader := c.GetHeader("Authorization")
		if authHeader == "" {
			c.Next()
		...
		if len(parts) == 2 && parts[0] == "Bearer" {
			claims, err := jwt.ParseToken(parts[1], cfg)
			if err == nil {
				session, sessionErr := sessionRepo.FindActive(claims.SessionID)
				if sessionErr != nil || session.UserID != claims.UserID { c.Next(); return }
				c.Set("user_id", claims.UserID)
				c.Set("email", claims.Email)
			}
		}
