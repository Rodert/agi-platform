import { BellOutlined, BulbOutlined, ClockCircleOutlined, CloseOutlined, ExportOutlined, FolderOpenOutlined, HomeOutlined, LogoutOutlined, MenuOutlined, SettingOutlined, TeamOutlined, ToolOutlined, UserOutlined, WalletOutlined } from '@ant-design/icons'
...
export function AppLayout(){
 ...
 const [drawer,setDrawer]=useState(false),[loginOpen,setLoginOpen]=useState(false),[communityOpen,setCommunityOpen]=useState(false),[noticeOpen,setNoticeOpen]=useState(false),[creditOpen,setCreditOpen]=useState(false),[promoVisible,setPromoVisible]=useState(true);const [msg,ctx]=message.useMessage()
 ...
 return <div className={`app-shell ${promoVisible?'promo-visible':''}`}>{ctx}
  {promoVisible&&<div className="promo-banner" role="banner"><a href="https://app.codex-zh.com/" target="_blank" rel="noopener noreferrer"><span>潮汐 AI 视频生成</span><ExportOutlined/></a><button type="button" aria-label="关闭推广横条" title="关闭" onClick={()=>setPromoVisible(false)}><CloseOutlined/></button></div>}
  ...
  <div className="app-content ml-16 max-[800px]:ml-0 max-[800px]:pt-16"><Outlet/></div>
