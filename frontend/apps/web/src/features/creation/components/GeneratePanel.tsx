...
export function GeneratePanel({onModeChange,refill}:{onModeChange?:(mode:CreationMode)=>void;refill?:{prompt:string;key:string|number}}={}){
 ...
 const isGPTImage2=current?.name==='gpt-image-2'
 ...
 const submit=async()=>{if(!requireAuth())return;if(!prompt.trim())return msg.warning('先描述你想生成的画面');if(!current)return msg.error('暂无可用图片模型');const result=await createTask({prompt:prompt.trim(),type:'image',modelName:current.name,params:modelParams,referenceImage:references[0]});if(result!=='success'){if(result==='failed')msg.error('任务提交失败');return}setPrompt('');setReferences([]);navigate('/create')}
 return <>{ctx}<section className="surface generator-panel overflow-hidden"><div className="flex min-h-[108px] gap-3 p-4"><ReferenceImagePicker value={references} onChange={setReferences}/><Input.TextArea autoSize={{minRows:3,maxRows:6}} variant="borderless" value={prompt} onChange={e=>setPrompt(e.target.value)} placeholder="描述你想生成的画面，AI 将为你生成精美图片..." className="!text-base"/><Tooltip title="优化提示词"><Button aria-label="优化提示词" type="text" shape="circle" icon={<HighlightOutlined/>} onClick={()=>void optimize()} loading={optimizing} disabled={optimizing}/></Tooltip></div><div className="generator-toolbar"><CreationModeDropdown mode="image" onChange={onModeChange}/><Select aria-label="图片模型" popupMatchSelectWidth={220} variant="borderless" value={model||undefined} placeholder="暂无可用模型" onChange={setModel} options={imageModels.map(x=>({label:modelLabel(x.display_name),value:x.name}))} className="image-model-select"/>{isGPTImage2&&<ImageSizePicker ratio={String(modelParams.ratio||'1:1')} resolution={String(modelParams.resolution||'1K')} onChange={(ratio,resolution)=>setModelParams({...modelParams,ratio,resolution})}/>}<ModelParams model={current} values={modelParams} onChange={setModelParams} compact excludeKeys={isGPTImage2?['ratio','resolution']:[]}/><span className="ml-auto text-[#8d8da2]">◆ <b className="text-[#b3b3c2]">{current?modelParamCost(current,modelParams):'-'}</b></span><Button type="primary" shape="circle" icon={<ArrowUpOutlined/>} onClick={()=>void submit()} disabled={!current}/></div></section></>
}
