...
export function ExplorePage(){
 ...
 const [mode,setMode]=useState<CreationMode>(requestedMode==='video'?'video':'image'),[type,setType]=useState('全部'),[query,setQuery]=useState(''),[searchTerm,setSearchTerm]=useState(''),[searchFocused,setSearchFocused]=useState(false),[active,setActive]=useState<Work|null>(null),[publishOpen,setPublishOpen]=useState(false)
 ...
 const list=useMemo(()=>works.filter(w=>(type==='全部'||(type==='图片'?w.type==='image':w.type==='video'))&&(!searchTerm||`${w.title}${w.prompt}`.toLowerCase().includes(searchTerm.toLowerCase()))),[works,type,searchTerm])
