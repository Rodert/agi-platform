...
export function VideoPage() {
  ...
  return <main className="page">{context}<h1 className="page-title">视频生成</h1><p className="page-subtitle">根据文字描述生成视频</p><div className="mt-6 grid grid-cols-[1fr_340px] gap-5 max-[900px]:grid-cols-1"><section className="surface p-5"><Input.TextArea rows={8} value={prompt} onChange={event => setPrompt(event.target.value)} placeholder="描述镜头运动、主体动作与环境变化..."/><div className="mt-4 flex justify-end"><Button type="primary" disabled={!current} onClick={() => void submit()}>生成视频 · {current ? modelParamCost(current,modelParams) : '-'} 灵感值</Button></div></section><aside className="surface p-5"><h3 className="mt-0">视频参数</h3><label className="theme-muted mb-2 block">模型</label><Select aria-label="视频模型" popupMatchSelectWidth={240} popupClassName="video-model-dropdown" value={current?.name} placeholder="暂无可用模型" onChange={setModel} options={videoModels.map(item => ({ label: <span className="video-model-option" title={item.display_name}>{item.display_name}</span>, value: item.name }))} className="video-model-select mb-5 w-full"/><ModelParams model={current} values={modelParams} onChange={setModelParams}/></aside></div></main>
}
